...
namespace YawVR
{
    ...
    */
    ...
    public static class Commands
    {
        ...
        {
            var orientationFormat = $"Y[{FormatRotation(yaw)}]P[{FormatRotation(pitch)}]R[{FormatRotation(roll)}]";
            var buzzerFormat = buzzer.isOn && buzzer.hz > 0 ? $"V[{buzzer.right_amp},{buzzer.center_amp},{buzzer.left_amp},{buzzer.hz}]"
                : "V[0,0,0,0]";
