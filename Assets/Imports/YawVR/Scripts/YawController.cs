...
namespace YawVR
{
    ...
    public class YawController : MonoBehaviour, YawControllerType, YawTCPClientDelegate, YawUDPClientDelegate
    {
        ...
        [SerializeField] Transform referenceTransform; // we will copy this objects rotation, and send it to the sim
        [SerializeField] string gameName; //name of the game
        ...
        [SerializeField] private Vector3 rotationMultiplier = new Vector3(1, 1, 1); // multiplier for YAWTracker
        ...
        private void Start()
        {
            ...
            if (connectType == ConnectType.ConnectViaIP)
            {
                ConnectToDevice(
                    new YawDevice(
                        IPAddress.Parse(debug_ipAddress),
                        DeviceType.YAW3,
                        50020,
                        50010,
                        "001",
        ...
        public void CalibrateDevice(bool allAxis)
        {
            if (state == ControllerState.Connected)
            {
                tcpCLient.BeginSend(
                    new byte[2]
                    {
                        Commands.CALIBRATE[0], (byte)(allAxis ? 1 : 0)
                    });
                //Set state to starting
            }
        }
        ...
        public void DidRecieveUDPMessage(string message, IPEndPoint remoteEndPoint)
        {
            ...
            if (message.Contains("YAWDEVICE"))
            {
                ...
                {
                    var header = messageParts[0].Trim();
                    DeviceType type = header switch {
                        "YAWDEVICE3" => DeviceType.YAW3,
                        "YAWDEVICE2" => DeviceType.YAW2,
                        _ => DeviceType.YAW1
                    };
        ...
        public void DidRecieveTCPMessage(byte[] data)
        {
            ...
            Debug.Log($"[TCP HEX - {data.Length} bytes] {hex}");
